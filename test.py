...
def test_db():
    db = Database()
    id = test(db.blogs.create_blog, {"title": "Test", "content": "Test", "author": "Test", "likes": [], "publish_status": True})["id"]
    test(db.blogs.get_blog)
    test(db.blogs.get_blog, id)
    test(db.blogs.update_blog, id, {"title": "new", "content": "Test", "author": "new", "likes": [], "publish_status": True})
