...
async def add_blog(req: Request, data: AddBlogSchema) -> Response:
    try:
        output: str = db.blogs.add_blog(data)
        return Response(
            dumps(
...
async def delete_blog(req: Request, id: str) -> Response:
    try:
        output: dict = db.blogs.delete_blog(id)
        return Response(
            dumps(output),
...
async def update_blog(req: Request, data: UpdateBlogSchema) -> Response:
    try:
        output: dict = db.blogs.update_blog(str(data.id), data.dict())
        return Response(
            dumps(output),
            status_code=200 if output["status"] else 404,
            media_type="application/json",
...
async def get_all(
    ...
    try:
        results: list = db.blogs.get_blogs(None, show_all=showall)
...
async def update_status(
    ...
    try:
        blog = db.blogs.get_blog_by_id(str(id))
        blog["blog_publish_status"] = blog_publish_status
        output: dict = db.blogs.update_blog(blog["id"], blog)
