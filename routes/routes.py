...
async def add_like(request: Request, id: str):
    if not request.session_user.get("_id"):
        return Response({"status": False, "message": "User not logged in"}, status_code=403)
...
async def add_comment(request: Request, comment_data: Comment):
    ...
    if request.session_user["_id"] != comment_data.author_id:
        return Response({"status": False, "message": "You are not authorized to add this comment"}, status_code=403)
...
async def add_bookmark(request: Request, blog_id: str):
    if not request.session_user.get("_id"):
        return Response({"status": False, "message": "User not logged in"}, status_code=403)
    return Response(await db.users.add_bookmark(blog_id=blog_id, user_id=request.session_user["_id"]))
